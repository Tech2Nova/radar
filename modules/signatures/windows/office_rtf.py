...
class RTFUnknownVersion(Signature):
    ...
    def on_complete(self):
        ...
        if "Rich Text Format data" in filetype and b"unknown version" in filetype:
            self.mark(
                filename=name,
            ...
            for dropped in self.get_results("dropped", []):
                if "filepath" in dropped:
                    ...
                    if b"Rich Text Format data" in droppedtype and b"unknown version" in droppedtype:
                        self.mark(
...
class RTFCharacterSet(Signature):
    ...
    def on_complete(self):
        ...
        filetype = target.get("file", {}).get("type") or ""
        name = target.get("file", {}).get("name")
        if "Rich Text Format data" in filetype and b"unknown character set" in filetype:
            self.mark(
                filename=name,
                filetype_details=filetype,
            )
            for dropped in self.get_results("dropped", []):
                if "filepath" in dropped:
                    droppedtype = dropped["type"]
                    droppedname = dropped["name"]
                    if b"Rich Text Format data" in droppedtype and b"unknown character set" in droppedtype:
                        self.mark(
                            dropped_filename=droppedname,
                            dropped_filetype_details=filetype,
                        )
