...
urlpatterns = [
    ...
    path("filtered/<int:task_id>/<int:pid>/<str:category>/", views.filtered_chunk, name='analysis.views.filtered_chunk'),  # 过滤数据块
    ...
    re_path(r"^moloch/(?P<ip>[\d\.]+)?/(?P<host>[a-zA-Z0-9-_\.]+)?/(?P<src_ip>[a-zA-Z0-9\.]+)?/(?P<src_port>\d+|None)?/(?P<dst_ip>[a-zA-Z0-9\.]+)?/(?P<dst_port>\d+|None)?/(?P<sid>\d+)?$", views.moloch, name='analysis.views.moloch'),
