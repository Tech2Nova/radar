...
class Database(object):
    ...
    def list_machines(self, locked=False):
        ...
        try:
            if locked:
                machines = session.query(Machine).options(joinedload(Machine.tags)).filter_by(locked=True).all()
            else:
                machines = session.query(Machine).options(joinedload(Machine.tags)).all()
            return machines
        except SQLAlchemyError as e:
    ...
    def view_task(self, task_id, details=True):
        ...
        try:
            if details:
                task = session.query(Task).options(joinedload(Task.guest), joinedload(Task.errors), joinedload(Task.tags)).get(task_id)
            else:
    ...
    def view_machine(self, name):
        ...
        session = self.Session()
        try:
            machine = session.query(Machine).options(joinedload(Machine.tags)).filter_by(name=name).first()
        except SQLAlchemyError as e:
    ...
    def view_machine_by_label(self, label):
        ...
        session = self.Session()
        try:
            machine = session.query(Machine).options(joinedload(Machine.tags)).filter_by(label=label).first()
        except SQLAlchemyError as e:
