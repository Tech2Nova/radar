...
URL_REGEX = (
    ...
    br"((["
    # IP address.
    br"(?:[0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\\."
    br"(?:[0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\\."
    br"(?:[0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\\."
    br"(?:[0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])]|"
    # Or domain name.
...
class File(object):
    ...
    def get_exported_functions(self):
        ...
        pe32_type = self.get_type()
        if isinstance(pe32_type, bytes):
            pe32_type = pe32_type.decode()

        if "MS-DOS" not in filetype and "PE32" not in pe32_type and "PE32+" not in pe32_type:
            return
    ...
    def get_imported_functions(self):
        ...
        filetype = self.get_type()
        if "MS-DOS" not in filetype and "PE32" not in self.get_type() and "PE32+" not in self.get_type():
            return
